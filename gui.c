...
DWORD WINAPI AutoUpdateThread(LPVOID lpParam) {
    ...
    long long lastUpdate = g_lastSubUpdateTime;
    ...
    double intervalSeconds = 24.0 * 3600.0; // 默认每天

    if (mode == UPDATE_MODE_WEEKLY) {
        intervalSeconds = 7.0 * 24.0 * 3600.0;
    } else if (mode == UPDATE_MODE_CUSTOM) {
        intervalSeconds = (double)interval * 3600.0;
    }

    double diffSeconds = difftime(now, (time_t)lastUpdate);

    // 如果上次更新有效且未到期，则跳过
    if (lastUpdate > 0 && diffSeconds < intervalSeconds) {
        double remainingHours = (intervalSeconds - diffSeconds) / 3600.0;
        log_msg("[Sub] 自动更新未到期，距离下次更新还需 %.1f 小时。", remainingHours);
        return 0;
    }

    // 3. 执行更新
    log_msg("[Sub] 满足更新条件，开始执行自动更新任务...");
    int count = UpdateAllSubscriptions(FALSE); 
    
    if (count >= 0) {
        // 更新成功（即使获取 0 个节点也算更新成功，避免死循环）
        SaveSettings(); // UpdateAllSubscriptions 内部已刷新 g_lastSubUpdateTime，此处持久化
